// Almanac query utilities for blockchain event data
//
// This crate provides utilities for querying blockchain event data
// with support for filtering, aggregation, and performance optimizations.

// Re-export indexer_core::Error for convenience
pub use indexer_core::Error;
pub use indexer_core::types::ChainId;
...
mod tests {
    ...
    fn test_base_query() {
        ...
        // Apply filters
        query.with_filter(filter.clone())
            .with_pagination(pagination.clone())
            .with_sorting(sorting.clone());
        
        // Verify the query is constructed correctly
        assert_eq!(query.filter.as_ref().unwrap().chain_ids.as_ref().unwrap()[0], ChainId::from("ethereum"));
        assert_eq!(query.pagination.as_ref().unwrap().limit, 10);
        assert_eq!(query.pagination.as_ref().unwrap().offset, 5);
        assert_eq!(query.sorting.as_ref().unwrap().field, "timestamp");
        assert_eq!(query.sorting.as_ref().unwrap().direction, SortDirection::Descending);
    }
} 