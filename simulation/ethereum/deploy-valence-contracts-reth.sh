#!/bin/bash
# Purpose: Deploy Valence contracts to the Reth Ethereum node
...
echo -e "${BLUE}=== Deploying Valence Contracts to Reth ===${NC}"
...
if ! curl -s -X POST -H "Content-Type: application/json" --data '{"jsonrpc":"2.0","method":"eth_chainId","params":[],"id":1}' http://localhost:8545 > /dev/null; then
    echo -e "${RED}Error: Reth node is not running at http://localhost:8545${NC}"
    echo -e "${YELLOW}Please start Reth first with: ./simulation/ethereum/setup-reth.sh${NC}"
    exit 1
fi
...
echo -e "${GREEN}✓ Reth chain ID: ${CHAIN_ID_DEC}${NC}"

# Configuration
DATA_DIR="data/contracts/ethereum/reth"
DEPLOYMENT_INFO="${DATA_DIR}/deployment-info.json"
CONTRACT_ADDRESSES_FILE="${DATA_DIR}/contract-addresses.env"

# Default private key for Reth/Anvil (account 0)
...
deploy_and_record() {
    ...
    local output
    if [ -z "$args" ]; then
        output=$(forge create ${contract_path} --rpc-url ${RPC_URL} --private-key ${PRIVATE_KEY} --chain-id ${CHAIN_ID_DEC} --json)
    else
        output=$(forge create ${contract_path} --rpc-url ${RPC_URL} --private-key ${PRIVATE_KEY} --chain-id ${CHAIN_ID_DEC} --constructor-args ${args} --json)
    fi
...
echo "# Valence contract addresses on Reth" > ${CONTRACT_ADDRESSES_FILE}.tmp
...
# Set gateway's relayer (use the default Reth/anvil account for testing)
