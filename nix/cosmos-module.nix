...
{ self, inputs, ... }:
{
  ...
  perSystem = { config, self', inputs', pkgs, system, ... }: {
    ...
    packages = {
      ...
      wasmd-setup = pkgs.runCommand "wasmd-setup" {
        script = pkgs.writeText "wasmd-setup.sh" ''
          #!/usr/bin/env bash
          set -euo pipefail
          ...
          # Create dummy wasmd script file
          mkdir -p $GOPATH/bin
...
EOF
        ...
        # Create a wrapper
        mkdir -p $out/bin
        cp $script $out/bin/wasmd-setup
        chmod +x $out/bin/wasmd-setup
        
        # Add runtime dependencies to the wrapper
      ...
      wasmd-node = pkgs.runCommand "wasmd-node" {
        script = pkgs.writeText "wasmd-node.sh" ''
          #!/usr/bin/env bash
          set -euo pipefail
      ...
      test-cosmos-adapter = pkgs.runCommand "test-cosmos-adapter" {
        script = pkgs.writeText "test-cosmos-adapter.sh" ''
          #!/usr/bin/env bash
          set -euo pipefail
        ...
        # Create a wrapper
        mkdir -p $out/bin
        cp $script $out/bin/test-cosmos-adapter
