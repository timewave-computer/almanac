...
use ethers::{
    prelude::*,
    providers::{Http, Provider, Middleware, StreamExt, Ws},
    signers::{LocalWallet, Signer},
    types::{BlockNumber, Address, U256, TransactionRequest},
...
impl RethClient {
    ...
    async fn load_contract(&self, address_str: &str) -> Result<ContractInfo> {
        // Parse address
    ...
    async fn find_contract_abi(&self, dir: &Path, contract_name: &str) -> Result<Option<ethers::abi::Abi>> {
        ...
        for entry in fs::read_dir(dir)? {
            ...
            if path.is_dir() {
                ...
                if let Some(abi) = self.find_contract_abi(&path, contract_name).await? {
                    return Ok(Some(abi));
                }
    ...
    async fn find_contract_abi_by_scanning(&self, dir: &Path) -> Result<Option<ethers::abi::Abi>> {
        ...
        for entry in fs::read_dir(dir)? {
            ...
            if path.is_dir() {
                // Recursively search subdirectories
                if let Some(abi) = self.find_contract_abi_by_scanning(&path).await? {
                    return Ok(Some(abi));
                }
    ...
    /// Query contract state
    async fn query_contract(&self, address_str: &str, function: &str, args_str: &str) -> Result<()> {
    ...
    async fn call_contract_function(
        ...
        let tokens = self.json_args_to_tokens(args, &func.inputs)?;
        
        // Call function
        let result = contract
            .method::<_, ethers::contract::Multicall>(function, tokens)?
            .call()
            .await?;
    ...
    async fn send_transaction(
        ...
        println!("Transaction mined in block: {}", receipt.block_number.unwrap_or_default());
    ...
    /// Listen for contract events
    async fn listen_for_events(&self, address_str: &str, event_name: &str, blocks: u64) -> Result<()> {
        ...
        let to_block = if blocks == 0 { 
            BlockNumber::Latest.into() 
        } else { 
            (current_block + blocks).into() 
        };
